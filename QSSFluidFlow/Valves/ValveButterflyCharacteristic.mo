model ValveButterflyCharacteristic 
  "Valve with butterfly characteristic and exponential first order lag dynamics"
  ...
  annotation (
    ...
    Icon(
      ...
      Text(
        ...
        style(color=41)), 
      Text(extent=[-100, 54; 100, 40], string="Butterfly")), 
    Window(
      x=0.14, 
      y=0.01, 
      width=0.66, 
...
equation 
  ...
  kv = noEvent(if (x >= xmin) then kvs*(1 - cos((pi/2)*x)) else kvs*(1 - cos((
    pi/2)*x)));
  
  //constitutive equation of valve
  dp = noEvent(if (q > 0) then (1/kv^2)*kSI*q^2 else -(1/kv^2)*kSI*(-q)^2);
end ValveButterflyCharacteristic;
